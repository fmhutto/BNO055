...
bool BNO055::write8(bno055_reg_t reg, uint8_t value) {
  int err = wiringPiI2CWriteReg8(_handle, reg, value);
  /* ToDo: Check for errors! */
  return true;
}

uint8_t BNO055::read8(bno055_reg_t reg) {
  uint8_t value = wiringPiI2CReadReg8(_handle, reg);
  /* ToDo: Check for errors! */
  return value;
}

/*!
 *  @brief  Reads the specified number ofstd::bytes over I2C
 */
bool BNO055::readLen(bno055_reg_t reg,uint8_t *buffer, uint8_t len) {
  int n = wiringPiI2CReadBlockData(_handle, reg, buffer, len);
